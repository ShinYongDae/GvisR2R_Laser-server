...
int _export WINAPI addnoise(int amount, imgdes far *srcimg, imgdes far *desimg);
int _export WINAPI addtext(unsigned int color, int pointsize, LPCSTR font, LPCSTR atext, imgdes far *desimg);
int _export WINAPI addtextex(unsigned int color, LOGFONT *lfont, LPCSTR atext, imgdes far *desimg);
int _export WINAPI convert8bitto1bitex(int mode, imgdes far *srcimg, imgdes far *desimg, UCHAR *screen);
...
int _export WINAPI dropbits(const int value, imgdes far *srcimg, imgdes far *desimg);
int _export WINAPI emboss(imgdes far *srcimg, imgdes far *desimg);
int _export WINAPI embossongray(imgdes far *srcimg, imgdes far *desimg);
int _export WINAPI extractcolorrange(RGBTRIPLE *rgbcolor, int extent, imgdes far *srcimg,imgdes far *desimg);
int _export WINAPI freebuffer(UCHAR *bufferaddress);
int _export WINAPI gaussianblur(int ksize, imgdes far *srcimg, imgdes far *desimg);
unsigned int  _export WINAPI getbuffersize(UCHAR *bufferaddress);
int _export WINAPI makepalette(imgdes far *image, PALETTEPOINT * ppa, int numelem);
int _export WINAPI outlineongray(imgdes far *srcimg, imgdes far *desimg);
int _export WINAPI rgbimagetocmykimage(imgdes far *srcimg, imgdes far *desimg);
int _export WINAPI cmykimagetorgbimage(imgdes far *srcimg, imgdes far *desimg);

int _export WINAPI screen(imgdes far *srcimg, imgdes *oprimg, imgdes far *desimg);
int _export WINAPI tile(imgdes far *srcimg, imgdes far *desimg);
...
int  _export WINAPI expandcontrast(int min, int max, imgdes far *srcimg, 
   imgdes far *desimg);
int _export WINAPI extractplane(int plane, imgdes far *srcimg, imgdes far *desimg);
...
int  _export WINAPI hsvimagetorgbimage(imgdes far *srcimg, imgdes far *desimg);
...
int  _export WINAPI insertplane(int plane, imgdes far *srcimg, imgdes far *desimg);
...
int  _export WINAPI loadpngwithalpha(LPCSTR afname, imgdes far *desimg, imgdes *alphaImg);
int  _export WINAPI loadpngwithalphaW(LPCWSTR wcfname, imgdes far *desimg, imgdes *alphaImg);
...
int  _export WINAPI loadtifpagebyindexfrombuffer(UCHAR huge *buff, imgdes far *desimg, int pageIndex);
...
int  _export WINAPI rgbimagetohsvimage(imgdes far *srcimg, imgdes far *desimg);
int  _export WINAPI rotate(const double angle, imgdes far *srcimg, imgdes far *desimg);
int  _export WINAPI rotateex(const double angle, imgdes far *srcimg, imgdes far *desimg, int mode);
...
int  _export WINAPI savegifmultiframetobuffer(UCHAR huge * far *buffAddr, imgdes *imageArr, GifGlobalSaveData *gdata, 
   GifFrameSaveData *fdataArr, int mode, int nelem);
...
int  _export WINAPI savepngtobuffer(UCHAR huge * far *buffAddr, imgdes *srcimg, int compsn);
int  _export WINAPI savepngtobufferex(UCHAR huge * far *buffAddr, imgdes *srcimg, int compsn, int mode, PNG_DATA *pData);
...
int  _export WINAPI savetifpage(LPCSTR fname, imgdes far *srcimg, int cmprsn, int page);
int  _export WINAPI savetifpageW(LPCWSTR fname, imgdes far *srcimg, int cmprsn, int page);
...
int  _export WINAPI tiffgetpageinfofrombuffer(UCHAR huge *buff, int far *totalPages, int far *pageArray, int arrayElems);
...
int  _export WINAPI tiffinfopagebyindexfrombuffer(UCHAR huge *buff, TiffData far *tdat, int pageIndex);

int  _export WINAPI tiffinfopagebyindexex(LPCSTR fname, TiffDataEx far *tdatex, int pageIndex);
int  _export WINAPI tiffinfopagebyindexexW(LPCWSTR fname, TiffDataEx far *tdatex, int pageIndex);
int  _export WINAPI tiffinfopagebyindexfrombufferex(UCHAR huge *buff, TiffDataEx far *tdatex, int pageIndex);
...
int  _export WINAPI wtaverage(const int prct, imgdes *srcimg, 
   imgdes *oprimg, imgdes *desimg);
...
int  _export WINAPI TWgetpendingxfers(HWND hWnd, int far *count);
...
int  _export WINAPI TWscanmultipleimages(HWND hWnd, imgdes far *desimg, int (WINAPI *saveScan)(imgdes *));
...
int  _export WINAPI checkrange32_(imgdes far *image);
